...
import { hasUrlParameter, addUrlParameter } from "../query/index.h5";
import { isJingGouMiniprogram } from "../platform/index.h5";
// import { goToLogin } from '@dj-lib/dj-base-lib/build/login/index.h5';

//唤起app的登录窗口
export const goToLogin = (opts: Record<string, string>) => {
  // 京东购物小程序
  return new Promise((resolve, reject) => {
    const returnurl = opts?.returnurl;
    const paramsappId = opts?.appId;
    const appId = paramsappId || 121;
    const url = returnurl || globalThis.location.href;
    if (isJingGouMiniprogram) {
      if (!hasUrlParameter(url, "hasLogin")) {
        globalThis.location.href = `https://wq.jd.com/pinbind/pintokenredirect?biz=jdms&url=${encodeURIComponent(addUrlParameter(url, "hasLogin", "1"))}`;
      }
      resolve(true);
    } else {
      // 京东登录
      const passport =
        "https://plogin.m.jd.com/user/login.action?appid=" +
        appId +
        "&returnurl=";
      const returnval = passport + encodeURIComponent(url);
      globalThis.location.href = returnval;
      resolve(true);
    }
  });
};
