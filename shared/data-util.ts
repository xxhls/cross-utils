...
export const dataConvert = {
  ...
  colorToRgb(color: string): string {
    ...
    const rgbColor = globalThis.getComputedStyle(div).color;
    div.remove();
    return rgbColor;
  },
   ...
   */
  base64: {
    encode: (str: string): string => globalThis?.btoa(str),
    decode: (str: string): string => globalThis?.atob(str),
  },
...
export const objectUtils = {
  ...
  hasOwn<T extends unknown = {}, K extends string[] = []>(
    ...
    for (const key of keys) {
      ...
      const t = typeof obj[key as string];
...
export const imageUtils = {
  ...
  format: {
    ...
    advanced(
      ...
      // Base64图片直接返回
      if (url.indexOf("data:image") === 0 && url.includes("base64")) {
        return url;
      }
      ...
      const idx =
        (Number.parseInt(url.substr(url.lastIndexOf("/") + 1, 8), 36) || 0) %
        pool.length;
      url = url.replace(
