    ...
    return new Promise((resolve, reject) => {
        if(isJDApp){
            if(window.uuid){
                ...
                window.XWebView && window.XWebView.callNative("AppUnitePlugin","getAppInfo",JSON.stringify(jsonString),"getJDPhoneInfoByDJ","1");//后两个参数可选
            }     
            return;    
            ...
            const openuuid = cookie.get("openid") || cookie.get("__jda")?.split(".")[1] || "";
            resolve(openuuid);
            return;
        }
    })
