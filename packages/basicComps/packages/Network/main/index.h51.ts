...
import { isHourPRO, isJDApp, isJingGouMiniprogram, jdAppVersion } from "../Platform";
...
class Network implements INetwork {
    fetch(params: TFetchParams): Promise<any> {
        ...
        const getRequestId = (res)=>{
            ...
            const requestApiKey = keys.find(x => x?.toLowerCase() === 'x-api-request-id');
            const requestId = headers[requestApiKey!] || headers['x-api-request-id'] || headers['X-Api-Request-Id'];
            return requestId;
        }
        ...
        const _sucCallBack = (res,resolve, reject) => {
            ...
            if (code != NetReqErrorCode.success || !(body || result)) {
                ...
                if (code == NetReqErrorCode.success && (body && body.errorCode && body.errorCode != NetReqErrorCode.success)) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const {hybridDevIdNum,hybridIdNum,hybirdRequestId,hybirdDevRequestId} = params || {};
            ///获取过来判断是否有hybridId
            if(isJDApp && !!goHybrid && ((hybridDevIdNum && hybirdDevRequestId) || (hybirdRequestId && hybridIdNum))){
                
                //获取过来Hybird上的requestId
                const hybird_requestId = isHourPRO ? hybirdRequestId:hybirdDevRequestId ;
                const hybirdId = isHourPRO ? hybridIdNum : hybridDevIdNum ;
                hybridMutiPreload(hybirdId, hybird_requestId, api, newParmas).then((res) => {
                ...
                if(isJDApp){
                    ...
                    window.XWebView.callNative("ColorQueryPlugin","colorRequest",JSON.stringify(paramsJson),"callback","1");//后两个参数可选
                    window.callback = function(result){
                        _sucCallBack(result, resolve, reject) 
                    }
                    return ;
                }
                api(newParmas).then((res) => {
                    _sucCallBack(res, resolve, reject)
