...
import { isJDApp, isIOS } from "@basicComps-pkg/Platform/main";
...
const setJdNavi = (params?, type?) => {
  ...
  if (isIOS) {
    ...
    window?.MobileNavi &&
      window.MobileNavi.configNavigationBar(JSON.stringify(json));
  }
};
...
const setNavMore = (haveShare) => {
  ...
  if (isIOS) {
    ...
    window?.MobileNavi &&
      window.MobileNavi.configBtnSince610(JSON.stringify(Json));
  }
};
const NavAPI = {
  setJdNavi,
