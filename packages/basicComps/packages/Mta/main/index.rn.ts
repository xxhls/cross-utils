...
class Mta implements IMta {
    ...
    click(params: TClickParams): void {
        ...
        jsonParam = isType.isEmpty(jsonParam) ? null : dataConvert.stringify.value(jsonParam) as any;
    ...
    pv(params: TPVParams): void {
        ...
        ext = isType.isEmpty(ext) ? {} : Object.assign(ext as any, { "hasPvExtend": "true" });
    ...
    addTag(pageId: string, source: string, sourceValue: string): void {
        ...
        JDMta.addTag(tagParams).then((response) => {
    ...
    setStdParam(params: STDParams): void {
        ...
        JDMta.setUserProperty(params).then((response) => {
