...
const sha256 = (input) => {
    ...
    const coreSha256 = (m, l) => {
        ...
        for (let i = 0; i < m.length; i += 16) {
            ...
            for (let j = 0; j < 64; j++) {
                ...
                const T1 = safeAdd(safeAdd(safeAdd(safeAdd(h, sigma1256(e)), ch(e, f, g)), K[j]), W[j]);
    ...
    const output = utf8Encode(input);
    return binb2hex(coreSha256(str2binb(output), output.length * chrsz));
}
