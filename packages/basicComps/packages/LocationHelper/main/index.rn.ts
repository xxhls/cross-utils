...
const isSupportLBSVersion = new Promise<boolean>((resolve) => {
  JDNativeSystem.getClientVersion()
    .then((version: string) => {
      resolve(compareVersions(IOS_SUPPORT_REMOTE_JSON_VERSION, version) != -1);
    })
    .catch(() => {
      resolve(false);
    });
...
const isLowVersionList = new Promise<boolean>((resolve) => {
  JDNativeSystem.getClientVersion()
    .then((version: string) => {
      resolve(compareVersions(SUPPORT_REMOTE_JSON_VERSION, version) != -1);
    })
    .catch(() => {
      resolve(false);
    });
...
class LocationHelper {
  ...
  async getAddressInfo(): Promise<{ data?: any; error?: any }> {
    ...
    return new Promise((resolve) => {
      JDLBS.getAddress(
        { businessId, isNeedDetail: "1", sceneId },
        (data) => {
          //返回 {lati,longi}
          resolve({ data });
        },
  ...
  async getCacheAddress(params?): Promise<{ data?: any; error?: any }> {
    ...
    return isSupportLBSVersion.then((support) =>
      support && svcType
  ...
  private _getLatLng() {
    return new Promise<{ success: boolean; data?: any; error?: any }>(
      (resolve) => {
        const t = setTimeout(() => {
        ...
        const getTimePosition = System.isIOS
          ? JDLBS.getAddress
          : JDLBS.getLatLng;
  ...
  getLBSCacheAddress = async (source?) => {
    ...
    const {
      ...
      showCityName,
      showAddressKey,
    } = (res ?? {}) as LBSLocationResult;

    if (code != "0") {
      return {
        type: E_AddressError.FailGetCache,
        msg: `获取 LBS 地址失败，${msg || code}`,
      };
    }
  ...
  getB2CCacheAddress = async () => {
    try {
      ...
      info.fourAddress =
        fullAddress ||
        `${provinceName}${cityName}${info.areaName}${townName}${addressDetail}`;
      // info.area = `${provinceId}_${cityId}_${info.areaId}_${townId}`;
      info.addressDetail =
        info?.showAddressName ||
        info.shortAddressName ||
        info.addressTitle ||
        info.detailAddress ||
        info.addressDetail ||
        info.fullAddress ||
  ...
  openAddressListPage = async (params?) => {
    ...
    return new Promise((resolve) => {
      ...
      const allUrl = isHighVersion
        ? "router://JDBWebAddressModule/pushWebView"
        : "router://JDBusinessAddressModule/pushAddressSelectViewController";
