...
class LocationHelper {
				...
				jdaddr.getAddressGlobal({
					...
					success(info) {
						if (info.status === '0') {
							...
							const { showAddressName, shortAddressName, addrDtl, fullAddress, cityName, provinceName, provinceId, cityId, countyId, townId } = info.data || {}
							const { provinceCode, cityCode, districtCode, townCode } = info.data || {}
							
							const addressDetail = showAddressName || shortAddressName || addrDtl || fullAddress || cityName || provinceName || ''
	
							let area  = '0_0_0_0'
							if (provinceId || cityId || countyId || townId) {
								area = [provinceId, cityId, countyId, townId].map(item => item || '0').join('_');
							} else {
								area = [provinceCode, cityCode, districtCode, townCode].map(item => item || '0').join('_');
							}
							...
							resolve({ address: info.data, addressDetail, area, latitude: info.data.latitude, longitude: info.data.longitude})
					...
					async fail(error) {
						...
						try {
							if (error.status == '-301') {
								...
								global.console.log('进入到了LBS中的_getAddress的的的的fail', res)
								// H5中获取缓存地址失败，先用缓存地址
	...
	openAddressListPage = (params): Promise<unknown> | undefined => {
		...
		if(!!isHarmonyOS){
			...
			return wx.miniProgram.navigateTo({url:`/pages/address/pages/location/index?token=${token}&sceneId=${sceneId}&from=${from}&rurl=${rurl}`})
